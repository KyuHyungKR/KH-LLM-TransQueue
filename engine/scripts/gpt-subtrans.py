...
def format_subtitle_batch(batch: List[pysrt.SubRipItem]) -> str:
    output = []
    for sub in batch:
        output.append(
...
def parse_translated_batch(batch: List[pysrt.SubRipItem], translated_text: str) -> List[pysrt.SubRipItem]:
    updated_subs = []
    for sub in batch:
...
def get_sub_batches(subs: pysrt.SubRipFile, max_batch_size: int, scene_threshold: float) -> List[List[pysrt.SubRipItem]]:
    batches = []
    current_batch = []
